...
M.course_dndupload = {
    ...
    init: function(Y, options) {
        ...
        div = this.add_status_div();

        if (!this.browser_supported()) {
            div.setContent(M.util.get_string('nofilereader', 'core_dndupload'));
            return; // Browser does not support the required functionality
        }

        if (!this.ajax_edit_enabled()) {
            div.setContent(M.util.get_string('noajax', 'core_dndupload'));
            return;
        }
    ...
    add_status_div: function() {
        ...
        var coursecontents = document.getElementById('page-content');
        if (coursecontents) {
    ...
    /**
     * Check that AJAX course editing is enabled
     * @return true if AJAX course editing enabled
     */
    ajax_edit_enabled: function() {
    ...
    get_section: function(el) {
        return el.ancestor( function(test) {
            return test.hasClass('section') && test.hasClass('main');
        }, true);
    },
    ...
    get_mods_element: function(section) {
        ...
        var modsel = section.one('ul.section');
        if (!modsel) {
    ...
    file_handler_dialog: function(handlers, extension, file, section, sectionnumber) {
        ...
        var uploadid = Math.round(Math.random()*100000);
        var content = '';
        var sel;
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    ...
                    alert('Server error'); // FIXME - replace with proper get_string
                }
            }
        };
     ...
     * @param sectionnumber the number of the selected course section
     */
    add_editing: function(elementid, sectionnumber) {
